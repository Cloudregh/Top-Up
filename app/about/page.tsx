...
export default function About() {
  return (
    <>
      ...
      <Section><div className="grid items-center gap-8 md:grid-cols-2">
      ...
      <Section title="Recognition"><ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{AWARDS.map((a) => <li key={a.t} className="tile p-5"><p className="font-semibold">{a.t}</p><p className="mt-1 text-xs text-muted">{a.by}</p></li>)}</ul></Section>
      <Section title="Where to find us"><ul className="grid gap-3 md:grid-cols-3">{LOCATIONS.map((l) => <li key={l.name} className="tile p-5"><p className="font-semibold">{l.name}</p><p className="mt-1 text-sm text-muted">{l.addr}</p></li>)}</ul></Section>
    </>
  );
}
