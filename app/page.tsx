...
import { AWARDS, CATEGORY_TILES, FAQS, INSURERS, SERVICES } from "@/lib/site";
  ...
  return (
    <>
      ...
      <section className="mt-20 px-4 sm:px-8">
        <Reveal><h2 className={`${H2} text-center`}>Our Popular Categories</h2></Reveal>
        <Reveal stagger={0.06} className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {CATEGORY_TILES.map((c) => (
            <Link key={c.label} href={`/shop?q=${encodeURIComponent(c.q)}`} className="group text-center">
              <div className="tile aspect-square overflow-hidden p-2"><Img k={c.img} w={400} h={400} alt={c.label} className="size-full rounded-[22px] object-cover transition duration-700 group-hover:scale-110" /></div>
        ...
        <Reveal stagger={0.1} className="mt-10 grid gap-4 md:grid-cols-3">
          {[
            ...
            <Link key={c.t} href={c.q ? `/shop?q=${c.q}` : "/appointment"} className="group relative block aspect-[4/5] overflow-hidden rounded-[28px] md:aspect-[3/4]">
              ...
              <div className="absolute inset-0 bg-gradient-to-b from-[#1b2260]/60 via-[#1b2260]/5 to-[#1b2260]/75" />
      ...
      <section className="mt-24 px-4 sm:px-8">
        <Reveal stagger={0.12} className="grid gap-4 md:grid-cols-2">
          <div className="relative flex min-h-[280px] overflow-hidden rounded-[28px] bg-[#1e2a9a] p-7 text-white">
            <div className="z-10 flex max-w-[55%] flex-col justify-between"><div><p className="text-xl font-semibold">Book an Appointment</p><p className="mt-2 text-xs text-white/70">Visit any of our branches and receive appropriate care specific to your health needs.</p></div>
              <Link href="/appointment" className="btn mt-6 w-fit bg-[#3345d0] text-white hover:bg-[#3f52e0]">Book an appointment <ArrowUpRight size={14} /></Link></div>
      ...
      {/* Services + insurers */}
      <section id="services" className="mt-24 scroll-mt-28 px-4 sm:px-8">
        ...
        <Reveal className="mt-10 text-center"><p className="text-sm font-semibold">Insurance partners</p><div className="mt-4 flex flex-wrap justify-center gap-2">{INSURERS.map((i) => <span key={i} className="rounded-full bg-[#f5f6fb] px-4 py-2 text-xs font-semibold">{i}</span>)}</div></Reveal>
      </section>
      ...
      {/* Delivery banner */}
      <section className="mt-24 px-3 sm:px-5">
