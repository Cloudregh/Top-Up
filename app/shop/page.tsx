...
function Shop() {
  ...
  const { items, loading, error, preview, hasMore, more, retry } = useCatalogue(q, category, 12);
  ...
  return (
    <div className="container-x py-8">
      ...
      {preview && <p className="mt-4 rounded-2xl bg-white/70 px-4 py-3 text-sm text-muted">Preview catalogue — <Link href={`/login?next=/shop`} className="font-semibold text-brand underline">sign in</Link> to see live stock and your own tier price, and to order.</p>}
      ...
      <div className="mt-6">
        {error && !items.length ? <ErrorState title="Couldn't load the catalogue" detail={error.detail || error.title} onRetry={retry} />
          ...
          : <>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">{items.map((p) => <ProductCard key={p.product_id} item={p} preview={preview} />)}</div>
